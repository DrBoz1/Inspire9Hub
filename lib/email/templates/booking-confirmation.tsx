...
import { colors, EmailAction, EmailDetail, EmailLayout, emailStyles, firstName, fontStack } from "../components/email-layout";
...
export default function BookingConfirmation({ memberName, memberEmail, roomName, location, bookingDate, startTime, endTime, durationHours, totalAUD, bookingRef, dashboardUrl, logoDataUrl }: BookingConfirmationProps) {
  return <EmailLayout preview={`Confirmed: ${roomName}, ${bookingDate}, ${startTime}–${endTime}.`} category="Your booking · Confirmed" title="Consider it reserved." logoDataUrl={logoDataUrl} recipient={memberEmail}>
    ...
    <Section style={{ backgroundColor: colors.soft, borderRadius: "10px", padding: "22px", margin: "12px 0 0" }}>
      <Row><Column><Text style={{ ...emailStyles.label, color: colors.positive, marginBottom: "5px" }}>Payment received</Text><Text style={emailStyles.muted}>Including GST</Text></Column>
        <Column align="right"><Text style={{ margin: 0, fontFamily: fontStack, fontSize: "26px", lineHeight: "34px", fontWeight: 500, letterSpacing: "-.6px", color: colors.ink }}>${totalAUD.toFixed(2)}</Text><Text style={{ ...emailStyles.label, margin: "3px 0 0", fontSize: "9px" }}>AUD</Text></Column></Row>
    </Section>
