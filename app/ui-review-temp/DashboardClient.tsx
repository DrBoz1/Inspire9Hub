...
export default function DashboardClient({ firstName, memberStatus, isInducted, isSubmitted, isAdmin, nextBooking, announcements, history }: DashboardClientProps) {
  ...
  return <div className="hub-dashboard">
    ...
    <div className="hub-feature-grid">
      ...
      <section className="hub-next-booking hub-surface">
        ...
        {nextBooking && bookingDate ? <>
          ...
          <div className="hub-empty-calendar" aria-hidden><CalendarDays size={38} strokeWidth={1} /></div>
    ...
    <div className="hub-membership-strip hub-surface">
      <Link href="/profile" className="hub-membership-item"><div><span className="hub-eyebrow">Your membership</span><strong><i className="hub-status-dot" data-active={isActive} />{memberStatus} member</strong></div><ArrowUpRight size={16} /></Link>
    ...
    <div className="hub-feed-grid">
      <section className="hub-feed hub-surface">
        ...
        {history.length ? <div className="hub-activity-list">{history.map(entry => {
          return <div key={entry.id} className="hub-activity-item"><ActivityMark kind={entry.entry_type} tone={entry.tags === "Approved" ? "green" : entry.tags === "Rejected" ? "red" : "neutral"} /><div><strong>{entry.entry_type}</strong><p>{entry.entry_description ?? "Your hub activity has been updated."}</p></div><time>{entry.added_date ? format(parseISO(entry.added_date), "d MMM") : entry.entry_date ?? "—"}</time></div>;
